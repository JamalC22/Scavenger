...
function initiate_Handshake(startedresource)
	if startedresource == getThisResource() then
		triggerServerEvent("onPlayerHandshake", getLocalPlayer(), getPlayerName(getLocalPlayer()))
	end
end
addEventHandler("onClientResourceStart", getRootElement(), initiate_Handshake)
...
addEventHandler( "onClientGUIAccepted", loginPWEdit, 
	function (source)
		triggerServerEvent("onAttemptLogin", getLocalPlayer(), getPlayerName(getLocalPlayer()), guiGetText(loginPWEdit))
	end
)
...
addEventHandler( "onClientGUIAccepted", registerPWTwoEdit, 
	function (source)
		local pwOne = guiGetText(registerPWTwoEdit)
		local pwTwo = guiGetText(registerPWOneEdit)
		if pwOne == pwTwo then
			triggerServerEvent("onAttemptRegister", getLocalPlayer(), getPlayerName(getLocalPlayer()), pwOne)
		else
			outputChatBox("passwords dont match")
		end
	end
)
...
function showLoginGUI (isRegistered)
	...
	if isRegistered then
		...
		guiBringToFront(loginPWEdit)
		showCursor(true)
	else
		guiSetVisible(registerGUI,true)
		guiBringToFront(registerGUI)
		guiBringToFront(registerPWOneEdit)
		showCursor(true)
	end
end
